...
class crm_phonecall(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if context.get('stop_google_calendar_sync', False):
            event_id = super(crm_phonecall, self).create(cr, uid, vals, context)
            return event_id
        ...
        try:
            ...
            start_utc = dateutil.parser.parse(vals['date'])
            end_utc = dateutil.parser.parse(vals['date'])
            new_event = {'summary': vals['name'],
                             'description': vals.get('description', ''),
                             'location': vals.get('location', ''),
                             'start': {'dateTime': start_utc.strftime('%Y-%m-%dT%H:%M:%S.000Z')},
            ...
            vals['google_event_id'] = False
            logger.error('Insert google calendar event failed in create method for crm_phonecall.')
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if context.get('stop_google_calendar_sync', False):
            res = super(crm_phonecall, self).write(cr, uid, ids, vals, context=context)
            return res
        ...
        for id in ids:
            ...
            if any(k in vals for k in ['name', 'description', 'location', 'date', 'date_deadline']):
                google_sequence = self.read(cr, uid, id, ['google_sequence'])['google_sequence']
                vals['google_sequence'] = google_sequence + 1
                super(crm_phonecall, self).write(cr, uid, id, vals, context=context)
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        del_pool = self.pool.get('crm.phonecall.deleted')
...
class crm_phonecall_deleted(osv.osv):

    _name = 'crm.phonecall.deleted'
    _columns = {
            'google_event_id': fields.char('Google Calendar Event ID'),
            'user_id': fields.many2one('res.users', 'User'),
    }
...
class google_api_calendar(osv.osv):
    ...
    def do_synchronize_phonecall(self, cr, uid, ids, context=None):

        instance = self.browse(cr, uid, ids[0])

        phonecall_pool = self.pool.get('crm.phonecall')
        del_pool = self.pool.get('crm.phonecall.deleted')
        ...
        # build a list with all google-events-id present in crm_phonecall for this user
        google_event_ids = phonecall_pool.search(cr, uid, [('user_id', '=', instance.user_id.id), ('google_event_id', '!=', False)])
        data = phonecall_pool.read(cr, uid, google_event_ids, ['google_event_id'])
        google_event_ids = []
        for item in data:
            google_event_ids.append(item['google_event_id'])

        # first sync events from google with crm_phonecall
        page_token = None
        while True:
            events = service.events().list(calendarId=instance.calendar_id, pageToken=page_token).execute()
            for event in events['items']:
                ...
                if not is_deleted:
                    # check if google event already in openerp
                    crm_phonecall = phonecall_pool.search(cr, uid, [('user_id', '=', instance.user_id.id), ('google_event_id', '=', event['id'])])
                    if crm_phonecall:
                        if event.get('updated', False):
                            updated_google = dateutil.parser.parse(event['updated']).astimezone(pytz.utc).strftime('%Y-%m-%d %H:%M:%S')
                            updated_oe = phonecall_pool.read(cr, uid, crm_phonecall[0], ['write_date'])
                            updated_oe = dateutil.parser.parse(updated_oe['write_date']).strftime('%Y-%m-%d %H:%M:%S')
                            ...
                            if updated_google < updated_oe: # check if we have to overwrite google with data from crm_phonecall
                                updated_event = phonecall_pool.browse(cr, uid, crm_phonecall[0])
                                start_utc = dateutil.parser.parse(updated_event.date)
                                end_utc = dateutil.parser.parse(updated_event.date_deadline)
                                if updated_event.allday:
                                    tz = pytz.timezone(instance.user_id.tz) if instance.user_id.tz else pytz.utc
                                ...
                                phonecall_pool.write(cr, uid, updated_event.id, {'google_sequence': updated_event.google_sequence + 1}, context={'stop_google_calendar_sync': 'True'})
                                if updated_event.google_event_id in google_event_ids:
                                    google_event_ids.remove(updated_event.google_event_id)
                    ...
                    if crm_phonecall:
                        if event.get('updated', False):
                            ...
                            if updated_google > updated_oe:  # overwrite crm_phonecall with data from google
                        ...
                        phonecall_pool.create(cr, uid, {'user_id': instance.user_id.id,
                                                      'name': event.get('summary', ''),
                                                      'description': event.get('description'),
                                                      'date': start_utc.strftime('%Y-%m-%d %H:%M:%S'),
                                                      'date_deadline': end_utc.strftime('%Y-%m-%d %H:%M:%S'),
        ...
        # now sync new crm_phonecalls with google
        new_events = phonecall_pool.search(cr, uid, [('user_id', '=', instance.user_id.id), ('google_event_id', '=', False)])
        for new_event in phonecall_pool.browse(cr, uid, new_events):
            start_utc = dateutil.parser.parse(new_event.date)
            end_utc = dateutil.parser.parse(new_event.date_deadline)
            if new_event.allday:
                tz = pytz.timezone(instance.user_id.tz) if instance.user_id.tz else pytz.utc
            ...
            try:
                ...
                logger.error('Insert google calendar event failed for crm_phonecall id [%d] .' % (new_event.id))
                google_new_event = False
        ...
        # now delete all events from crm_phonecall which were not updated and/or created by google calendar, this ones are deleted in google calendar
        for gid in google_event_ids:
            delids = phonecall_pool.search(cr, uid, [('user_id', '=', instance.user_id.id), ('google_event_id', '=', gid)])
            phonecall_pool.unlink(cr, uid, delids, context={'stop_google_calendar_sync': 'True'})
